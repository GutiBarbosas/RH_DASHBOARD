...
(function App() {
  ...
  /* --------------------------------------------------------------------
   * 5f. HELPERS E RENDER DO MÓDULO DE BANCO DE HORAS
   * ------------------------------------------------------------------
   ...
   * @returns {Array<{COLABORADOR: string, LOJA: string, FUNCAO: string, sequenciaAtual: number}>}
   */
  function calcularSequenciasAtuais(registros) {
    const porColaborador = Utils.groupBy(registros, 'COLABORADOR');
    const resultado = [];

    Object.keys(porColaborador).forEach((colaborador) => {
      if (!colaborador || colaborador === 'undefined') return;

      const registrosOrdenados = [...porColaborador[colaborador]].sort((a, b) => {
  ...
  function renderBancoHoras(container) {
    ...
    const tableDiaWrap = Utils.el('div');
    ...
    container.appendChild(sectionHeader('Movimentação diária', 'Lançamentos individuais de horas normais e banco do dia, por colaborador'));
    container.appendChild(tableDiaWrap);
    ...
    function update(state) {
      ...
      // Movimentação diária: recebe também o filtro de STATUS, que só existe aqui.
      const filteredSaldoDia = Utils.filterByBusca(
        Filters.applyFilters(saldoDiaRes.data, state), busca, ['COLABORADOR']
      );
      ...
      sequenciaWrap.appendChild(dataTableCard({
        ...
        columns: ['Colaborador', 'Loja', 'Dias consecutivos'],
        rows: sequenciasFiltradas,
        loading,
        mapRow: (r) => [
          td(r.COLABORADOR), td(r.LOJA), tdTag(String(r.sequenciaAtual), sequenciaVariant(r.sequenciaAtual)),
        ],
      }));

      /* ------------------------------ Tabela: Movimentação diária ------------------------------ */
      Utils.emptyNode(tableDiaWrap);
      tableDiaWrap.appendChild(dataTableCard({
        title: 'Movimentação diária',
        columns: ['Data', 'Colaborador', 'Função', 'Loja', 'Horas normais', 'Banco do dia', 'Status'],
        rows: [...filteredSaldoDia].sort((a, b) => (b.DT ? b.DT.getTime() : 0) - (a.DT ? a.DT.getTime() : 0)),
        loading,
        mapRow: (r) => [
          td(Utils.formatDate(r.DT)), td(r.COLABORADOR), td(r.FUNCAO), td(r.LOJA),
          td(Utils.formatHoras(r.TOTAL_NORMAIS)), td(Utils.formatHoras(r.BANCO_TOTAL)), td(r.STATUS),
        ],
      }));
    }
    ...
    const filterApi = Filters.createFilterBar(filterWrap, [
      ...
      { key: 'busca', label: 'Colaborador', type: 'text', placeholder: 'Buscar por nome…' },
    ], (state) => { persistFilterState('banco-horas', state); update(state); });
  ...
  async function loadAllData(force = false) {
    ...
    const [{ admissoes, desligamentos, lojas }, rescisoes, ferias, quebraCaixa, lojaFuncao, saldoDia, saldoTotal] = await Promise.all([
