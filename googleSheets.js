...
const GoogleSheets = (() => {
  ...
  // BASE_QUEBRAS: COLABORADOR | VALOR | LOJA | GERENTE | SUPER | DT
  // (DT vem como abreviação de mês — "DEZ", "JAN" etc. — mapeado para MES,
  //  no mesmo padrão de texto usado pelas demais bases.)
  const QUEBRA_FIELDS = ['COLABORADOR', 'VALOR', 'LOJA', 'GERENTE', 'SUPERVISOR', 'MES'];

  // Campos que recebem tratamento especial de tipo (Date / Number / Moeda) após o mapeamento posicional.
  const DATE_FIELDS = new Set(['DATA']);
  const NUMBER_FIELDS = new Set(['HEADCOUNT']);
  // Campos monetários no formato brasileiro ("R$ 1.234,56") — tratamento próprio,
  // diferente de NUMBER_FIELDS, porque preserva os centavos (vírgula decimal).
  ...
  /** Converte valores monetários no formato brasileiro ("R$ 1.234,56") em Number (1234.56). */
  ...
  /** Busca a base de quebras de caixa. Ver contrato de retorno em _fetchDataset. */
