...
(function App() {
  ...
  const MODULE_REGISTRY = [
    ...
    { id: 'banco-horas', label: 'Banco de Horas', icon: 'bancoHoras', group: 'Em breve', status: 'soon' },
    { id: 'turnover', label: 'Turnover', icon: 'turnover', group: 'Gestão de Pessoas', status: 'active', render: renderTurnover },
    { id: 'absenteismo', label: 'Absenteísmo', icon: 'absenteismo', group: 'Em breve', status: 'soon' },
   ...
   */
  function exportActions({ title, columns, rows, mapRow }) {
    function extractExportRows() {
      return rows.map((row, i) => mapRow(row, i).map((cell) => cell.textContent));
    }
   ...
   */
  function dataTableCard({ title, columns, rows, mapRow, rowClass, onRowClick, loading = false, maxRows = 300 }) {
    ...
    card.appendChild(Utils.el('div', { class: 'table-card__header' }, [
      heading,
      (!loading && rows.length > 0) ? exportActions({ title, columns, rows, mapRow }) : Utils.el('div'),
    ]));
  ...
  /**
   * Lista de lojas "elegíveis" para compor um Headcount, dado o conjunto de
   * filtros ativos (LOJA/GERENTE/SUPERVISOR). Retorna null quando o filtro
   * "Função" está ativo — não há Headcount por Função na planilha, então
   * nenhum subconjunto de lojas representaria esse recorte com segurança.
   */
  function eligibleLojasForHeadcount(matchFilters, lojaResponsavelMap, lojasData) {
    if (matchFilters.FUNCAO) return null;
    let lojas = lojasData.map((l) => l.LOJA);
    if (matchFilters.LOJA) lojas = lojas.filter((l) => l === matchFilters.LOJA);
    if (matchFilters.GERENTE) lojas = lojas.filter((l) => lojaResponsavelMap[l] && lojaResponsavelMap[l].gerente === matchFilters.GERENTE);
    if (matchFilters.SUPERVISOR) lojas = lojas.filter((l) => lojaResponsavelMap[l] && lojaResponsavelMap[l].supervisor === matchFilters.SUPERVISOR);
    return lojas;
  }
  ...
  function renderTurnover(container) {
    ...
    container.appendChild(alertBanner({
      ...
      message: 'Turnover = ((Admissões finalizadas + Desligamentos) / 2) / Headcount médio × 100. O Headcount médio mensal é reconstruído a partir do Headcount atual da aba Lojas, recuando pelos eventos reais de admissão/desligamento registrados — nenhum valor histórico é presumido. Turnover por Função é exibido como contagem (não como taxa), pois a planilha não tem Headcount por Função. Lojas sem nenhuma movimentação no período ficam fora dos agrupamentos por Gerente/Supervisor, por não terem responsável confirmado nos dados.',
      variant: 'warning',
    }));
    ...
    const headcountPorLoja = {};
    lojasRes.data.forEach((l) => { if (l.LOJA) headcountPorLoja[l.LOJA] = l.HEADCOUNT || 0; });
    ...
    function update(state) {
      ...
      const eligibleLojas = eligibleLojasForHeadcount(groupFilters, lojaResponsavelMap, lojasRes.data);
      const headcountAtualFiltrado = eligibleLojas === null ? null : eligibleLojas.reduce((sum, l) => sum + (headcountPorLoja[l] || 0), 0);

      const geralAgg = eligibleLojas === null ? null : aggregateGroupTurnover(admissoesGroup, desligamentosGroup, headcountAtualFiltrado, periodFilter);
      // Evolução mensal sempre mostra a tendência completa, independente do filtro de Mês (só o KPI/tabela recorta).
      const geralSeriesCompleta = eligibleLojas === null ? [] : reconstructMonthlySeries(admissoesGroup, desligamentosGroup, headcountAtualFiltrado);
      ...
      kpiWrap.appendChild(kpiGrid([
        {
          ...
          value: (geralAgg && geralAgg.turnoverPct !== null) ? Utils.formatPercent(geralAgg.turnoverPct) : '—',
          delta: groupFilters.FUNCAO ? 'Sem Headcount por função — taxa não calculável' : '',
          loading,
        },
        ...
        },
        {
          ...
          value: headcountAtualFiltrado === null ? '—' : Utils.formatNumber(headcountAtualFiltrado),
          delta: groupFilters.FUNCAO ? 'Sem Headcount por função' : '',
          loading,
        },
        {
      ...
      if (eligibleLojas === null || !geralSeriesCompleta.length) {
        chartsWrapEvolucao.appendChild(Utils.el('p', {
          class: 'view__section-desc',
          text: eligibleLojas === null
            ? 'Não é possível traçar a evolução mensal com o filtro de Função ativo (sem Headcount por função).'
            : 'Sem movimentações no período/filtro selecionado para montar a evolução mensal.',
        }));
      } else {
      ...
      // Função: só contagem (sem Headcount por função na planilha) — mesmo padrão de groupMovStats usado no Dashboard.
      const funcaoStatsContagem = groupMovStats(admissoesExibicao, desligamentosExibicao, 'FUNCAO');
      ...
      chartsWrapDistribuicao.appendChild(realChartGrid([
        ...
        {
          title: 'Admissões x Desligamentos por função (contagem)',
          type: 'donut',
          data: {
            labels: funcaoStatsContagem.map((s) => s.key),
            datasets: [{ data: funcaoStatsContagem.map((s) => s.admissoes + s.desligamentos), backgroundColor: funcaoStatsContagem.map((_, i) => Charts.PALETTE[i % Charts.PALETTE.length]) }],
          },
        },
      ]));
  ...
  async function loadAllData(force = false) {
    ...
    const [{ admissoes, desligamentos, lojas }, rescisoes, ferias, quebraCaixa] = await Promise.all([
