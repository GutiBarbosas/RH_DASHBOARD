...
(function App() {
  ...
  const MODULE_REGISTRY = [
    ...
    { id: 'ferias', label: 'Férias', icon: 'ferias', group: 'Em breve', status: 'soon' },
    { id: 'banco-horas', label: 'Banco de Horas', icon: 'bancoHoras', group: 'Em breve', status: 'soon' },
    { id: 'turnover', label: 'Turnover', icon: 'turnover', group: 'Em breve', status: 'soon' },
  ...
  function exportActions({ title, columns, rows, mapRow }) {
    function extractExportRows() {
      return rows.map((row) => mapRow(row).map((cell) => cell.textContent));
    }
  ...
  function dataTableCard({ title, columns, rows, mapRow, rowClass, onRowClick, loading = false, maxRows = 300 }) {
    ...
    if (loading) {
      ...
      rows.slice(0, maxRows).forEach((row) => {
        ...
        const tr = Utils.el('tr', extraClass ? { class: extraClass } : {}, mapRow(row));
  ...
  function mesesPresentes(data) {
    const found = Utils.uniqueValues(data, 'MES');
  ...
  /**
   * Agrupa uma lista por um campo e soma um campo numérico (ex.: VALOR), retornando
   * itens ordenados do maior para o menor valor acumulado. Genérico — usado pelos
   * rankings de Quebra de Caixa (colaborador, loja, supervisor), mas serve para
   * qualquer base que precise de "maior acumulado por campo X".
   */
  function rankByValor(list, groupField, valorField = 'VALOR') {
    const grouped = Utils.groupBy(list, groupField);
    return Object.keys(grouped)
      .filter((k) => k && k !== 'undefined')
      .map((key) => ({
        key,
        valor: grouped[key].reduce((sum, r) => sum + (r[valorField] || 0), 0),
        ocorrencias: grouped[key].length,
      }))
      .sort((a, b) => b.valor - a.valor);
  }

  /** Card de ranking por valor acumulado (moeda) — mesma linguagem visual do rankingCard(), mas com valor em R$ e contagem de ocorrências. */
  function currencyRankingCard({ title, items, emptyText = 'Nenhum dado no período selecionado.' }) {
    const card = Utils.el('div', { class: 'ranking-card' });
    card.appendChild(Utils.el('div', { class: 'ranking-card__header' }, [
      Utils.el('h3', { class: 'ranking-card__title', text: title }),
    ]));
    ...
    if (!items.length) {
      ...
      items.forEach((item, idx) => {
        const pct = Math.max(4, Math.round((item.valor / max) * 100));
        list.appendChild(Utils.el('div', { class: 'ranking-item' }, [
          Utils.el('span', { class: `ranking-item__pos${idx < 3 ? ' ranking-item__pos--top' : ''}`, text: String(idx + 1) }),
          Utils.el('div', { class: 'ranking-item__body' }, [
            Utils.el('div', { class: 'ranking-item__top' }, [
              Utils.el('span', { class: 'ranking-item__name', text: item.key }),
              Utils.el('span', { class: 'ranking-item__count', text: Utils.formatCurrency(item.valor) }),
            ]),
            Utils.el('div', { class: 'ranking-item__bar' }, [
              Utils.el('div', { class: 'ranking-item__bar-fill', style: `width:${pct}%` }),
            ]),
            Utils.el('div', { class: 'ranking-item__meta' }, [
              Utils.el('span', { text: `${Utils.formatNumber(item.ocorrencias)} ocorrência${item.ocorrencias === 1 ? '' : 's'}` }),
            ]),
          ]),
        ]));
      });
  ...
  function renderQuebraCaixa(container) {
    ...
    container.appendChild(sectionHeader('Tendências'));
    container.appendChild(chartsWrap);
    container.appendChild(sectionHeader('Ranking das maiores quebras', 'Valor acumulado no período, por colaborador, loja e supervisor'));
    container.appendChild(rankingWrap);
    ...
    function update(state) {
      ...
      const mediaPorOcorrencia = filtered.length ? totalValor / filtered.length : 0;

      /* ------------------------------ KPIs ------------------------------ */
      const porColaboradorKpi = Utils.groupBy(filtered, 'COLABORADOR');
      const colaboradorNomes = Object.keys(porColaboradorKpi).filter((k) => k && k !== 'undefined');
      const topColaborador = colaboradorNomes
        .map((nome) => ({ nome, valor: porColaboradorKpi[nome].reduce((s, r) => s + (r.VALOR || 0), 0) }))
      ...
      kpiWrap.appendChild(kpiGrid([
        ...
        { icon: 'coin', label: 'Média por ocorrência', value: Utils.formatCurrency(mediaPorOcorrencia), loading },
        {
          icon: 'coin',
      ...
      const lojasOrdenadas = Object.keys(porLoja)
        .filter((k) => k && k !== 'undefined')
        .sort((a, b) => porLoja[b].reduce((s, r) => s + (r.VALOR || 0), 0) - porLoja[a].reduce((s, r) => s + (r.VALOR || 0), 0))
        .slice(0, 10);
      ...
      chartsWrap.appendChild(realChartGrid([
        {
          title: 'Valor de quebra por loja',
          type: 'bar',
          data: {
            labels: lojasOrdenadas,
            datasets: [{
              label: 'Valor (R$)',
              data: lojasOrdenadas.map((l) => porLoja[l].reduce((s, r) => s + (r.VALOR || 0), 0)),
      ...
      /* ------------------------------ Ranking das maiores quebras ------------------------------ */
      Utils.emptyNode(rankingWrap);
      rankingWrap.appendChild(rankingGrid([
        currencyRankingCard({ title: 'Ranking por colaborador', items: rankByValor(filtered, 'COLABORADOR').slice(0, 10) }),
        currencyRankingCard({ title: 'Ranking por loja', items: rankByValor(filtered, 'LOJA').slice(0, 10) }),
        currencyRankingCard({ title: 'Ranking por supervisor', items: rankByValor(filtered, 'SUPERVISOR').slice(0, 10) }),
      ]));
  ...
  async function loadAllData(force = false) {
    ...
    const [{ admissoes, desligamentos, lojas }, rescisoes, quebraCaixa] = await Promise.all([
